...
class CalcAlcohol:
    ...
    def get_gram_of_alcohol(amount, percent_alcohol):
        return ml_to_oz(amount) * 29.5735 * (percent_alcohol / 100) * 0.789

    @staticmethod
    def get_oz_of_alcohol(amount, percent_alcohol):
        result = get_gram_of_alcohol(amount, percent_alcohol)
        return gram_to_oz(result)
        
    @staticmethod
    def miligram_alc_pr_hundred_ml(amount, percent_alcohol):
        result = get_gram_of_alcohol(amount, percent_alcohol)
        return gram_to_miligram(result)